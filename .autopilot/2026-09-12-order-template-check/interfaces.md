...
### Из таска 04 — расхождение общих полей коносамента: ошибка, не предупреждение

- `checkBillGroups` в `template-check/core.js` (правило A07/G03): находка теперь `level:'error', fix:'customer'` в `ctx.data` (было `warning` в `ctx.warnings`); публичная форма `Finding` не изменилась. Гашение повторных находок того же (коносамент, поле) убрано — красится и попадает в отчёт **каждая** расходящаяся строка, не только первая; первая строка группы остаётся эталоном сравнения и сама не помечается.
- Тесты: `node --test template-check/core.test.js` — 57 → 60.
