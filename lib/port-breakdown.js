...
//
// splitEmpty (по умолчанию false, как и раньше) отделяет порожние
// контейнеры (вес груза 0) от гружёных того же типа в том же порту —
// тем же приёмом, что уже применён в buildTypeBreakdown ниже для Итогового
// отчёта. Опция не включена по умолчанию, чтобы не менять картину для
// вызовов, которые её не просили (см. dashboard-departure/core.js) — итог
// по порту и общий итог складываются по каждой строке независимо от этой
// опции, поэтому от неё не меняются.
function buildBreakdown(rows, { splitEmpty = false } = {}) {
  ...
  for (const row of rows) {
    ...
    const typeKey = splitEmpty ? `${row.type} ${isEmpty ? '1' : '0'}` : row.type;
    if (!portEntry.types.has(typeKey)) {
      portEntry.types.set(typeKey, { type: row.type, isEmpty, group: emptyGroup() });
    }
    const typeEntry = portEntry.types.get(typeKey);

    addToGroup(portEntry.group, row.container, row.cargoWeight, row.tareWeight, row.totalWeight);
    addToGroup(typeEntry.group, row.container, row.cargoWeight, row.tareWeight, row.totalWeight);
  ...
  const ports = Array.from(portMap.entries())
    ...
    .map(([port, entry]) => ({
      ...
      types: Array.from(entry.types.values())
        .sort((a, b) => a.type.localeCompare(b.type, 'ru'))
        .map(({ type, isEmpty, group }) =>
          splitEmpty ? { type, isEmpty, ...finalizeGroup(group) } : { type, ...finalizeGroup(group) },
        ),
    }));
 ...
 * @param {{portKeyword: string, portLabel: string, splitEmpty?: boolean}} portColumn — какую колонку
 *        порта искать («назначения»/«Порт назначения» или «отправления»/«Порт отправления»);
 *        splitEmpty (по умолчанию false) отделяет порожние контейнеры от
 *        гружёных того же типа — см. buildBreakdown
...
export function buildPortBreakdownDashboard(workbooks, { portKeyword, portLabel, splitEmpty = false }) {
  if (!Array.isArray(workbooks) || workbooks.length === 0) {
    return { ok: false, error: 'Нужен хотя бы один файл манифеста' };
  }
  ...
  for (const { fileName, workbook } of workbooks) {
    ...
    perFile.push({ fileName, breakdown: buildBreakdown(extracted.rows, { splitEmpty }) });
    allRows.push(...extracted.rows);
  }

  return { ok: true, perFile, combined: buildBreakdown(allRows, { splitEmpty }) };
}
