...
function buildTypeBreakdown(rows) {
  ...
  for (const row of rows) {
    if (!typeMap.has(row.type)) {
      typeMap.set(row.type, emptyGroup());
    }
    addToGroup(typeMap.get(row.type), row.container, row.cargoWeight, row.tareWeight, row.totalWeight);
    addToGroup(grandGroup, row.container, row.cargoWeight, row.tareWeight, row.totalWeight);
  }

  const byType = Array.from(typeMap.entries()).map(([type, group]) => ({ type, ...finalizeGroup(group) }));
 ...
 *
 * @param {Array<{fileName: string, workbook: import('exceljs').Workbook}>} workbooks
 * @returns {{ok: true, byType: Array<{type: string, count: number, cargoWeight: number, tareWeight: number, totalWeight: number}>,
