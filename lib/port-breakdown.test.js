...
test('buildPortBreakdownDashboard: без splitEmpty гружёные и порожние одного типа — по-прежнему одна группа', () => {
  const wb = buildWorkbook([
    { port: 'X', type: '40HC', cargo: 100, tare: 10, total: 110 },
    { port: 'X', type: '40HC', cargo: 0, tare: 5, total: 5 },
  ]);
  const result = buildPortBreakdownDashboard([{ fileName: 'a.xlsx', workbook: wb }], DESTINATION);
  assert.equal(result.ok, true);
  const [port] = result.combined.ports;
  assert.equal(port.types.length, 1);
  assert.deepEqual(port.types[0], { type: '40HC', count: 2, cargoWeight: 100, tareWeight: 15, totalWeight: 115 });
  assert.equal(port.types[0].isEmpty, undefined);
});

test('buildPortBreakdownDashboard: splitEmpty:true — порожние (вес груза 0) того же типа отдельной строкой от гружёных', () => {
  const wb = buildWorkbook([
    { port: 'X', type: '40HC', cargo: 100, tare: 10, total: 110 },
    { port: 'X', type: '40HC', cargo: 200, tare: 20, total: 220 },
    { port: 'X', type: '40HC', cargo: 0, tare: 5, total: 5 },
  ]);
  const result = buildPortBreakdownDashboard([{ fileName: 'a.xlsx', workbook: wb }], { ...DESTINATION, splitEmpty: true });
  assert.equal(result.ok, true);
  const [port] = result.combined.ports;
  const loaded = port.types.find((t) => t.type === '40HC' && !t.isEmpty);
  const empty = port.types.find((t) => t.type === '40HC' && t.isEmpty);
  assert.deepEqual(loaded, { type: '40HC', isEmpty: false, count: 2, cargoWeight: 300, tareWeight: 30, totalWeight: 330 });
  assert.deepEqual(empty, { type: '40HC', isEmpty: true, count: 1, cargoWeight: 0, tareWeight: 5, totalWeight: 5 });
  // итог по порту складывает обе строки, независимо от того, разбиты они или нет
  assert.deepEqual(port.totals, { count: 3, cargoWeight: 300, tareWeight: 35, totalWeight: 335 });
});

test('buildPortBreakdownDashboard: splitEmpty:true — в порту только порожние одного типа (без единого гружёного) — одна строка, не ломается', () => {
  const wb = buildWorkbook([{ port: 'X', type: '40HC', cargo: 0, tare: 5, total: 5 }]);
  const result = buildPortBreakdownDashboard([{ fileName: 'a.xlsx', workbook: wb }], { ...DESTINATION, splitEmpty: true });
  assert.equal(result.ok, true);
  const [port] = result.combined.ports;
  assert.equal(port.types.length, 1);
  assert.deepEqual(port.types[0], { type: '40HC', isEmpty: true, count: 1, cargoWeight: 0, tareWeight: 5, totalWeight: 5 });
});
...
test('buildManifestTotals: группирует только по типу контейнера, без порта вовсе', () => {
  ...
  assert.deepEqual(type40hc, { type: '40HC', isEmpty: false, count: 2, cargoWeight: 300, tareWeight: 30, totalWeight: 330 });
  assert.deepEqual(result.grandTotal, { count: 3, cargoWeight: 350, tareWeight: 35, totalWeight: 385 });
});

test('buildManifestTotals: порожний контейнер (вес груза 0) того же типа — отдельная строка, а не одна группа с гружёным', () => {
  const wb = buildTotalsWorkbook([
    { type: '40HC', cargo: 100, tare: 10, total: 110 },
    { type: '40HC', cargo: 0, tare: 20, total: 20 },
    { type: '40HC', cargo: 0, tare: 30, total: 30 },
  ]);
  ...
  const loaded = result.byType.find((t) => t.type === '40HC' && !t.isEmpty);
  const empty = result.byType.find((t) => t.type === '40HC' && t.isEmpty);
  assert.deepEqual(loaded, { type: '40HC', isEmpty: false, count: 1, cargoWeight: 100, tareWeight: 10, totalWeight: 110 });
  assert.deepEqual(empty, { type: '40HC', isEmpty: true, count: 2, cargoWeight: 0, tareWeight: 50, totalWeight: 50 });
  // Общий итог не зависит от разбивки на гружёные/порожние — считается по всем строкам сразу.
  assert.deepEqual(result.grandTotal, { count: 3, cargoWeight: 100, tareWeight: 60, totalWeight: 160 });
});
