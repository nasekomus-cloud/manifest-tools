// Единственное место, которое правится при добавлении нового инструмента (R04/A01).
// Обычный (не ES-модульный) скрипт: index.html должен открываться напрямую
// двойным кликом (file://), а браузеры блокируют CORS для <script type="module">,
// загружаемых с file:// — поэтому здесь без import/export, просто глобальный массив.
//
// Порядок записей — это и порядок в боковой панели, и «маршрут» (обычный порядок
// работы) на главной, и порядок карточек, внутри каждой вкладки (category).
// Больше порядок нигде не задан: переставить инструменты — переставить записи здесь.
//
// Поля записи:
//   title       — полное название (заголовок страницы, карточка на главной);
//   navTitle    — короткое имя для панели и маршрута (нет — берётся title);
//   description — описание на карточке;
//   href        — путь от корня сайта;
//   category    — вкладка: 'export' | 'import';
//   output      — метка формата результата: 'xlsx' | 'pdf' | 'zip'.
window.TOOLS = [
  {
    title: 'Проверить шаблон эл. поручения',
    navTitle: 'Проверить шаблон',
    description:
      'Сверить шаблон эл. поручения с формой для загрузки в систему и с поручением на погрузку: ошибки — жёлтым, к ним текстовый отчёт, а что можно — исправленный шаблон.',
    href: 'template-check/index.html',
    category: 'export',
    output: 'xlsx',
  },
  {
    title: 'Объединить манифесты',
    navTitle: 'Объединить манифесты',
    description:
      'Склеить несколько файлов манифеста в один общий файл, сохранив оформление.',
    href: 'merge/index.html',
    category: 'export',
    output: 'xlsx',
  },
  {
    title: 'Сверить опасные грузы',
    navTitle: 'Сверить опасные грузы',
    description:
      'Сверить сводный файл с официальным DG-манифестом по номеру контейнера.',
    href: 'dg-check/index.html',
    category: 'import',
    output: 'xlsx',
  },
  {
    title: 'Дашборд по портам отправления',
    navTitle: 'Дашборд по портам отправления',
    description:
      'То же самое, но разбивка по портам отправления (погрузки), а не назначения.',
    href: 'dashboard-departure/index.html',
    category: 'import',
    output: 'xlsx',
  },
  {
    title: 'Сверить с поручениями на погрузку',
    navTitle: 'Сверить с поручениями',
    description:
      'Сверить сводный файл с поручениями: вес груза и тары, пломбы, тип контейнера, номер поручения и класс опасного груза — несовпадения подсветятся жёлтым.',
    href: 'order-check/index.html',
    category: 'export',
    output: 'xlsx',
  },
  {
    title: 'Итоговый PDF-отчёт по манифесту',
    navTitle: 'Итоговый отчёт',
    description:
      'Сводная таблица «GRAND TOTAL» по типам контейнеров в PDF. Позывной, дату прибытия и терминал — впишите вручную.',
    href: 'grand-total/index.html',
    category: 'export',
    output: 'pdf',
  },
  {
    title: 'Сводный реестр поручений и коносаментов',
    navTitle: 'Реестр поручений и коносаментов',
    description:
      'Собрать из одного или нескольких манифестов таблицу уникальных пар «номер поручения — номер коносамента».',
    href: 'bl-registry/index.html',
    category: 'export',
    output: 'xlsx',
  },
  {
    title: 'Разделить коносаменты',
    navTitle: 'Разделить коносаменты',
    description:
      'Разбить один PDF с несколькими коносаментами на отдельные файлы по номеру, архивом zip.',
    href: 'bl-split/index.html',
    category: 'import',
    output: 'zip',
  },
  {
    // Тот же инструмент, что и выше, в «Импорте» — коносаменты разделяют и на
    // экспорте, вторая запись на ту же страницу, без второй копии кода.
    title: 'Разделить коносаменты',
    navTitle: 'Разделить коносаменты',
    description:
      'Разбить один PDF с несколькими коносаментами на отдельные файлы по номеру, архивом zip.',
    href: 'bl-split/index.html',
    category: 'export',
    output: 'zip',
  },
  {
    title: 'Информация по портам назначения',
    navTitle: 'Информация по портам назначения',
    description:
      'Разбивка по портам назначения: сколько контейнеров каждого типа и сколько это весит, порожние — отдельной строкой.',
    href: 'dashboard/index.html',
    category: 'export',
    output: 'xlsx',
  },
];
